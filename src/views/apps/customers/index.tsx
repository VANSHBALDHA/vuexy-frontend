...
import CityAndState from "../leads/statescity.json"
...
const Roles = () => {
  ...
  const getLeadsData = async () => {
    try {
      const res = await fetch('http://localhost:3001/customers-list', { cache: 'no-store' })
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      const resdata = await fetch(selectedCustomerId
        ? `http://localhost:3001/customers-update/${selectedCustomerId}`
  ...
  const handleDelete = (id: string) => {
    fetch(`http://localhost:3001/customers-delete/${id}`, {
      method: 'DELETE',
      headers: {
    ...
    <Grid container spacing={6}>
      ...
      >
        <div className='flex items-center justify-between plb-5 pli-6'>
          <Typography variant='h5'>{selectedCustomerId ? 'Edit Customer' : 'Add New Customer'}</Typography>
